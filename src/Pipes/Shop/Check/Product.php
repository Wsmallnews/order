...
class Product implements CheckPipeInterface
{
    public function check(OrderRocket $rocket, Closure $next): OrderRocket
    {
        ...
        foreach ($products as $key => &$buyInfo) {
            ...
            $buyInfo['relate_stock_num'] = $product['sku_type'] == 'unit' ? ($buyInfo['relate_num'] * ($currentVariant['convert_num'] ?: 1)) : $buyInfo['relate_num'];    // 基础库存数量

            // 当前购买规格
            if ($product['stock_type'] == 'stock' && $currentVariant['stock'] < $buyInfo['relate_num']) {
